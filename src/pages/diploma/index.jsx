...
import DiplomaView from './components/view'
  ...
  const handleSubmit = async ( e ) => {
    try {
      ...
      if ( diplomaRef.current ) {
        ...
        link.href = image;
        link.download = `${name} uchun diplom`; // File name for the downloaded certificate
        link.click();
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="max-w-md w-full">
        ...
        <DiplomaView diplomaRef={diplomaRef} name={name} />
      </div>
    </div>
  )
}